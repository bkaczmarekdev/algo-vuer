...
import type CountingSort from '@/algorithms/CountingSort/CountingSort'

const props = defineProps<{
  algoState: CountingSort
}>()
...
watch(
  () => props.algoState.currentCountingStep,
  (newStep) => {
    if (newStep) {
      currentStep.value = newStep
    } else {
      currentStep.value = {
        array: Array(Math.max(...props.algoState.input) + 1).fill(0),
        workingIndex: null,
      }
    }
  },
